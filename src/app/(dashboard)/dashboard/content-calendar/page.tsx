    ...
    <div className="p-4 lg:p-8 space-y-6 max-w-7xl mx-auto min-h-screen bg-[var(--surface-base)]">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          ...
          <p className="text-[var(--text-secondary)] text-sm mt-1">İçerik planınızı aylık ve liste görünümlerinde yönetin.</p>
        </div>
        <div className="flex bg-[var(--surface-base)] p-1 rounded-lg border border-[var(--border-subtle)]">
          <Button variant="ghost" size="sm" className={`h-8 px-4 ${viewMode==='calendar' ? 'bg-[var(--surface-overlay)] text-[var(--accent)] border-b-2 border-transparent' : 'text-[var(--text-tertiary)]'}`} onClick={() => setViewMode('calendar')}>Takvim</Button>
          <Button variant="ghost" size="sm" className={`h-8 px-4 ${viewMode==='list' ? 'bg-[var(--surface-overlay)] text-[var(--accent)] border-b-2 border-transparent' : 'text-[var(--text-tertiary)]'}`} onClick={() => setViewMode('list')}>Liste</Button>
        </div>
      </div>

      <div className="flex flex-wrap gap-3 bg-[var(--surface-raised)] p-4 rounded-[var(--radius-lg)] border border-[var(--border-subtle)]">
        <Select value={fStatus} onValueChange={(v) => setFStatus(v ?? '')}>
          <SelectTrigger className="w-32 h-9 bg-[var(--surface-overlay)] border-[var(--border-default)] text-[var(--text-primary)] rounded-[var(--radius-md)]"><SelectValue placeholder="Durum" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={fPlatform} onValueChange={(v) => setFPlatform(v ?? '')}>
          <SelectTrigger className="w-32 h-9 bg-[var(--surface-overlay)] border-[var(--border-default)] text-[var(--text-primary)] rounded-[var(--radius-md)]"><SelectValue placeholder="Platform" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={fFormat} onValueChange={(v) => setFFormat(v ?? '')}>
          <SelectTrigger className="w-32 h-9 bg-[var(--surface-overlay)] border-[var(--border-default)] text-[var(--text-primary)] rounded-[var(--radius-md)]"><SelectValue placeholder="Format" /></SelectTrigger>
          <SelectContent>
            {['Tümü', 'Reel', 'Carousel', 'Story', 'Post'].map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
          </SelectContent>
        </Select>
        <div className="flex-1" />
        <Button onClick={() => handleDayClick(new Date())} className="bg-[var(--accent)] text-white hover:bg-[var(--accent-hover)] rounded-[var(--radius-md)]">+ Yeni Klasör</Button>
      </div>

      {viewMode === "calendar" ? (
        <div className="bg-[var(--surface-raised)] border border-[var(--border-subtle)] rounded-[var(--radius-lg)] overflow-hidden">
          ...
          <div className="grid grid-cols-7 border-b border-[var(--border-subtle)]">
            {['Pzt', 'Sal', 'Çar', 'Per', 'Cum', 'Cmt', 'Paz'].map(d => (
              <div key={d} className="p-3 text-center text-xs font-semibold tracking-wider uppercase text-[var(--text-secondary)] border-r border-[var(--border-subtle)] last:border-0">{d}</div>
            ))}
          </div>
          <div className="grid grid-cols-7 auto-rows-[120px]">
            {getDays().map((day, i) => {
              ...
              return (
                <div key={i} onClick={() => handleDayClick(day)} className={`p-2 border-r border-b border-[var(--border-subtle)] cursor-pointer hover:bg-[var(--surface-overlay)] transition-colors ${!isSameMonth(day, currentDate) ? 'bg-[var(--surface-base)] opacity-50' : 'bg-[var(--surface-raised)]'}`}>
                  ...
                  <div className="space-y-1">
                    ...
                    {dayItems.length > 3 && <div className="text-[9px] text-[var(--text-secondary)] pl-1">+{dayItems.length - 3} daha</div>}
                  </div>
                </div>
              )
            })}
        ...
        <div className="bg-[var(--surface-raised)] border border-[var(--border-subtle)] rounded-[var(--radius-lg)] overflow-hidden divide-y divide-[var(--border-subtle)]">
          {filteredItems.sort((a,b) => new Date(a.scheduled_date).getTime() - new Date(b.scheduled_date).getTime()).map(item => (
            <div key={item.id} className="p-4 flex items-center gap-4 hover:bg-[var(--surface-overlay)] transition-colors text-[var(--text-primary)]">
      ...
      <Sheet open={panelOpen} onOpenChange={setPanelOpen}>
        <SheetContent className="w-[400px] sm:w-[540px] bg-[var(--surface-raised)] border-l border-[var(--border-subtle)] overflow-y-auto">
          <SheetHeader className="mb-6 border-b border-[var(--border-subtle)] pb-4">
            <SheetTitle className="text-[var(--text-primary)]">İçerik Ekle / Görüntüle</SheetTitle>
            {selectedDay && <p className="text-sm text-[var(--text-secondary)]">{format(selectedDay, "dd MMMM yyyy, EEEE", { locale: tr })}</p>}
          </SheetHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea value={form.caption || ""} onChange={e => setForm({...form, caption: e.target.value})} className="bg-[var(--surface-overlay)] border-[var(--border-default)] text-[var(--text-primary)] min-h-[100px] rounded-[var(--radius-md)] focus:border-[var(--accent)]" />
            </div>
            <div>
              <label className="text-xs text-[var(--text-secondary)] mb-1 block">Tarih</label>
              <Input type="date" value={form.scheduled_date || ""} onChange={e => setForm({...form, scheduled_date: e.target.value})} className="bg-[var(--surface-overlay)] border-[var(--border-default)] text-[var(--text-primary)] rounded-[var(--radius-md)] focus:border-[var(--accent)]" />
            </div>
            <Button onClick={handleSave} className="w-full bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white mt-4 rounded-[var(--radius-md)]">Kaydet</Button>
          </div>
        </SheetContent>
      </Sheet>
